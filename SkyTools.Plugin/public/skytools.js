(function () {
  ...
  var SKYTOOLS_UI_VERSION = "2026-07-25-fab-edge-collapse-1";
  ...
  if (window.__skytoolsPluginLoaded && window.__skytoolsPluginVersion !== SKYTOOLS_UI_VERSION) {
    ...
    var staleNodes = document.querySelectorAll(".skytools-panel,.skytools-fab-shell,.skytools-fab,.skytools-toast,.skytools-game-button");
  ...
  var INSTALLED_STORAGE_KEY = "SkyTools.InstalledSnapshot";
  ...
  function readInstalledSnapshot() {
    try {
      ...
      if (savedAt && Date.now() - savedAt > INSTALLED_SNAPSHOT_TTL) {
        return null;
      }
      return parsed;
    } catch (_) {
  ...
  function ensureFloatingMenu() {
    if (!document.body || document.querySelector(".skytools-fab-shell")) {
      return;
    }

    var shell = document.createElement("div");
    shell.className = "skytools-fab-shell";
  ...
  function updateFloatingMenu() {
    ...
    if (toggle) {
      toggle.innerHTML = icon(collapsed ? "chevronLeft" : "chevronRight");
  ...
  function runAsyncAction(title, startMethod, resultMethod, payload, after) {
    ...
    setBusy(title, "Executando no console oculto.");
    return call(startMethod, payload).then(function (result) {
      if (!isPendingResult(result)) {
        return completeActionResult(title, result, after);
      }
      var data = normalizeData(result) || {};
      setActivity("busy", title, data.message || "Executando no console oculto.");
  ...
  function preloadStartupData(force) {
    ...
    window.setTimeout(function () {
      Promise.all([
        ...
        refreshFixGames(force === true),
        refreshApisCache(force === true)
      ]).then(function () {
        updateGameButton();
        if (document.querySelector(".skytools-panel")) {
          renderPanelBody();
  ...
  function openPanel() {
    ...
    panel.addEventListener("click", function (event) {
      ...
      if (action === "installed") runAction("Carregando biblioteca", "SkyToolsInstalled", {}, function (result) { state.installed = result; updateNameCacheFromInstalled(result); writeInstalledSnapshot(result); state.activeTab = "biblioteca"; });
      ...
      if (action === "fix-prepare") {
        ...
        runAsyncAction("Aplicando Sky", "SkyToolsApplyFix", "SkyToolsApplyFixResult", fixPayload, function (result) {
          var data = normalizeData(result) || {};
