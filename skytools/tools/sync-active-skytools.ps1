...
if (-not $isAdmin) {
    ...
    Start-Process -FilePath $powerShellHost -Verb RunAs -WindowStyle Minimized -ArgumentList @(
        '-NoProfile',
...
$copies = @(
    ...
    @{ Source = 'backend\skytools_fix_index.json'; Target = 'backend\skytools_fix_index.json' },
    @{ Source = 'backend\skytools_fixes.js'; Target = 'backend\skytools_fixes.js' },
    @{ Source = 'backend\skytools_installed.js'; Target = 'backend\skytools_installed.js' },
    @{ Source = 'backend\skytools_installer.js'; Target = 'backend\skytools_installer.js' },
    @{ Source = 'backend\skytools_names.js'; Target = 'backend\skytools_names.js' },
    @{ Source = 'backend\skytools_steam_installed.js'; Target = 'backend\skytools_steam_installed.js' },
...
$themeSourceDir = Join-Path $sourceRoot 'public\themes'
function Copy-SkyToolsThemes {
    param(
        [Parameter(Mandatory = $true)][string]$Destination
    )

    if (-not (Test-Path -LiteralPath $themeSourceDir)) {
        return
    }
    ...
    Get-ChildItem -LiteralPath $themeSourceDir -Filter '*.css' -File -ErrorAction SilentlyContinue | ForEach-Object {
        $dst = Join-Path $Destination $_.Name
        Copy-Item -LiteralPath $_.FullName -Destination $dst -Force

        $srcHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $_.FullName).Hash
        $dstHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $dst).Hash
        if ($srcHash -ne $dstHash) {
            throw "Falha ao sincronizar tema $($_.Name): o arquivo ativo ficou diferente da fonte."
        }

        Write-Host "OK tema $dst"
    }
}

Copy-SkyToolsThemes -Destination (Join-Path $targetRoot 'public\themes')
Copy-SkyToolsThemes -Destination (Join-Path $targetRoot 'webkit\SkyTools\themes')
...
Copy-SkyToolsThemes -Destination (Join-Path $steamUiSkyToolsRoot 'themes')
